...
export function Events() {
  ...
  if (isLoading) {
    return (
      <section id="events" className="bg-gray-50 flex flex-col">
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 w-full flex-1 flex flex-col py-16 md:py-32">
          <div className="mb-12 md:mb-16">
            <span className="text-sm uppercase tracking-[0.3em] text-gray-400">Upcoming</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 flex-1">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="animate-pulse">
                <div className="bg-gray-200 h-64 md:h-full mb-4 md:mb-6"></div>
              </div>
            ))}
          </div>
        </div>
        ...
        {events.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {events.map((post: IBoardPost, index: number) => (
              ...
              >
                <div className="relative aspect-[3/4] overflow-hidden bg-gray-200 mb-4 md:mb-6">

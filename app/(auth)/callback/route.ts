...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    console.log("Exchange result:", { data: !!data?.session, error });
    if (error) {
  ...
  if (redirectTo) {
    return NextResponse.redirect(
      `${origin}${redirectTo}${redirectTo.includes("?") ? "&" : "?"}oauth_success=true`
    );
  }

  if (next !== "/") {
    return NextResponse.redirect(
      `${origin}${next}${next.includes("?") ? "&" : "?"}oauth_success=true`
    );
  }
